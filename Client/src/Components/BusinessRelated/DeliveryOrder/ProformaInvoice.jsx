...
const ProformaInvoice = ({ doc_no, disabledFeild }) => {
  ...
  const fetchData = async () => {
        try {
            ...
            const response = await fetch(
                `${API_URL}/generating_proforma_invoice?Company_Code=${companyCode}&Year_Code=${Year_Code}&doc_no=${doc_no}`
            );
            if (!response.ok) {
...
const shouldUseImage =
  docDate >= cnameUpdatedDate
    ...
    pdf.text(`${allData.BillFooter}`, 45, 38);
      ...
      pdf.text("Proforma Invoice", 95, 49);
 ...
 return (
        <div id="pdf-content">
            {pdfPreview && <PdfPreview pdfData={pdfPreview} apiData={invoiceData[0]} label={"ProformaInvoiceForDO"} />}
            <button
                onClick={fetchData}
                disabled={disabledFeild || loading}
                style={{
                    height: '38px',
                    padding: '8px 16px',
                    backgroundColor: disabledFeild || loading ? '#cccccc' : '#1976d2',
                    color: '#ffffff',
                    border: 'none',
                    borderRadius: '4px',
                    cursor: disabledFeild || loading ? 'not-allowed' : 'pointer',
                    display: 'flex',
                    alignItems: 'center',
