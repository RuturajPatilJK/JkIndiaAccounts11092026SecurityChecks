...
const PurchaseTCSRegister = () => {
    ...
    const handleExportToExcel = () => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'PurchaseTCSRegister');
        XLSX.writeFile(wb, 'PurchaseTCSRegister.xlsx');
    };
    ...
    const generatePDF = async (companyName, fromDate, toDate, reportData) => {
        ...
        doc.setFontSize(10);
        doc.text(`Purchase TCS Register From: ${formatDate(fromDate)} To ${formatDate(toDate)}`, 10, 20);

        tableData.push(["PAN", "Party Name", "Taxable Amount", "CGST", "SGST", "IGST", "Bill Amount", "TDS Amount"]);
                ...
                <Table sx={{ minWidth: 650 }} aria-label="sales tcs register">
                    <TableHead>
                        <TableRow sx={{ backgroundColor: '#f5f5f5' }}>
                            <TableCell style={{ textAlign: "center", fontWeight: "bold" }}>Our No</TableCell>
                            <TableCell style={{ textAlign: "left", fontWeight: "bold" }}>PAN</TableCell>
                            ...
                            return (
                                <React.Fragment key={key}>
                                    ...
                                    {filteredItems.map((item, index) => (
                                        <TableRow>
                                            <TableCell className="fw-bold">{item.Pan}</TableCell>
                                            <TableCell className="fw-bold">{item.Name_Of_Party}</TableCell>
                                            <TableCell align="right" className="fw-bold">{item.Taxable_Amt}</TableCell>
                                            <TableCell align="right" className="fw-bold">{item.CGST}</TableCell>
                                            <TableCell align="right" className="fw-bold">{item.SGST}</TableCell>
                                            <TableCell align="right" className="fw-bold">{item.IGST}</TableCell>
                                            <TableCell align="right" className="fw-bold">{item.Bill_Amount}</TableCell>
                                            <TableCell align="right" className="fw-bold">{item.TCS}</TableCell>
                                        </TableRow>
                                    ))}
                                    <TableRow sx={{ backgroundColor: '#e0e0e0' }}>
                                        <TableCell colSpan={1} sx={{ fontWeight: 'bold', backgroundColor: "yellow" }}>{pan}</TableCell> {/* Left-align PAN */}
                                        <TableCell sx={{ fontWeight: 'bold', textAlign: 'left', backgroundColor: "yellow" }}>{PartyName}</TableCell> {/* Center-align Party Name */}
                                        <TableCell sx={{ fontWeight: 'bold', textAlign: 'right', backgroundColor: "yellow" }}>{formatReadableAmount(TotalTaxable_Amt.toFixed(2))}</TableCell>
                                        <TableCell sx={{ fontWeight: 'bold', textAlign: 'right', backgroundColor: "yellow" }}>{formatReadableAmount(CGSTAmt.toFixed(2))}</TableCell>
                                        <TableCell sx={{ fontWeight: 'bold', textAlign: 'right', backgroundColor: "yellow" }}>{formatReadableAmount(SGSTAmt.toFixed(2))}</TableCell>
                                        <TableCell sx={{ fontWeight: 'bold', textAlign: 'right', backgroundColor: "yellow" }}>{formatReadableAmount(IGSTAmt.toFixed(2))}</TableCell>
                                        <TableCell sx={{ fontWeight: 'bold', textAlign: 'right', backgroundColor: "yellow" }}>{formatReadableAmount(BillamountAmt.toFixed(2))}</TableCell>
                                        <TableCell sx={{ fontWeight: 'bold', textAlign: 'right', backgroundColor: "yellow" }}>{formatReadableAmount(TCSAmt.toFixed(2))}</TableCell>
                                    </TableRow>
                                </React.Fragment>
                            );
                        })}
...
export default PurchaseTCSRegister;
