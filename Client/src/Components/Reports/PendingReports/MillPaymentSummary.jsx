...
const MillPaymentSummary = () => {
    ...
    const handlePrint = () => {
        ...
        const formatRow = (cells, isHeader = false) => {
            return `<tr>
                ...
                let align = 'left';
                if ([1, 11].includes(index)) align = 'center';
                else if ([2, 3, 4, 5, 6, 7, 8, 9, 10].includes(index)) align = 'right';
        ...
        let content = `
            <html>
            <head>
                <title>Mill Payment Summary</title>
                <style>
            ...
            <body>
                <h2>${Company_Name}</h2>
                <h4>Mill Payment Summary - From ${formatDate(fromDate)} To ${formatDate(toDate)}</h4>
        `;
        ...
        Object.entries(groupedData).forEach(([parentKey, parentGroup]) => {
            ...
            content += `<table>
                <thead>
                    ${formatRow([
                'Tender No', 'Date', 'Qty', 'Mill Rate', 'Inc Mill GST Rate',
                'Mill Amount', 'Paid', 'Adj Amt', 'P Qty', 'Balance', 'B Qty', 'Payment Date'
            ], true)}
                </thead>
                <tbody>`;

            Object.values(parentGroup.subGroups).forEach((subGroupData) => {
                subGroupData.items.forEach(item => {
                    content += formatRow([
                        item.Tender_No,
                ...
                content += `
                    <tr class="total-row">
                        ...
                        <td>${formatReadableAmount(subGroupData.totalQty)}</td>
                        <td></td>
                        <td>${formatReadableAmount(subGroupData.frmmilladjamt)}</td>
                        <td>${formatReadableAmount(subGroupData.millamount)}</td>
                        <td>${formatReadableAmount(subGroupData.paidamount)}</td>
                        <td></td>
                        <td>${formatReadableAmount(subGroupData.paidqty)}</td>
                        <td>${formatReadableAmount(subGroupData.dispatched)}</td>
                        <td>${formatReadableAmount(subGroupData.DO)}</td>
                        <td></td>
                    </tr>
                `;
            });
            ...
            content += `
                <tr>
                    <td colspan="12" class="group-header" style="color:red;">
                        Grand Total: ${formatReadableAmount(parentGroup.Grandtotal)}
                    </td>
                </tr>
            </tbody></table>`;
        });
    ...
    const generatePDF = async () => {
        ...
        doc.setFontSize(8);
        const reportTitle = `Mill Payment Summary - From Date: ${formatDate(fromDate)} To Date: ${formatDate(toDate)}`;
        ...
        Object.entries(groupedData).forEach(([parentKey, parentGroup]) => {
            ...
            Object.values(parentGroup.subGroups).forEach((subGroupData) => {
                ...
                tableBody.push([
                    { content: 'Sub Totals', colSpan: 2, styles: { fontStyle: 'bold', halign: 'right' } },
            ...
            tableBody.push([{
                content: `Grand Total: ${formatReadableAmount(parentGroup.Grandtotal)}`,
                colSpan: 12,
                styles: {
                    halign: 'center',
                    fontStyle: 'bold',
    ...
    return (
        <div style={{marginTop:"-80px"}}>
            ...
            <Typography variant="h6" style={{ textAlign: 'center', fontSize: "20px", fontWeight: "bold" }}> Mill Payment Summary</Typography>
            ...
            <div className="mb-3 row align-items-center">
                <div className="col-auto">
                    ...
                    {pdfPreview && <PdfPreview pdfData={pdfPreview} apiData={reportData[0]} label={"MillPaymentSummary"} />}
            ...
            <TableContainer component={Paper} sx={{ maxHeight: '80vh', overflow: 'auto' }}>
                <Table stickyHeader size="small" aria-label="Mill Payment Summary">
                    <TableHead>
                        <TableRow>
                            {[
                                "Tender No", "Date", "Quintal", "Mill Rate", "Inc Mill GST Rate",
                                "Mill Amount", "Paid", "Adj Amount", "Purchase Quintal", "Balance", "Balance Quintal", "Payment Date"
                            ].map((header, idx) => (
                                <TableCell
                                    key={idx}
                                    sx={{
                    ...
                    <TableBody>
                        {Object.entries(groupedReportDataWithSubgroup).map(([parentKey, parentData]) => (
                            <React.Fragment key={parentKey}>
                                <TableRow>
                                    <TableCell colSpan={12} sx={{ backgroundColor: '#e0f0ff', fontWeight: 'bold', color: 'blue' }}>
                                        {parentKey}
                                    </TableCell>
                                </TableRow>
                                    ...
                                    <React.Fragment key={subGroupKey}>
                                        {subGroupData.items?.map((item, idx) => (
                                            <TableRow key={idx}>
                                                ...
                                                <TableCell align="right">{formatReadableAmount(adjamt(item.adjusted, item.millamount, item.Quantal, item.PartyDispQty))}</TableCell>
                                                <TableCell align="right">{formatReadableAmount(pqty(item.adjusted, item.millamount, item.Quantal, item.PartyDispQty, item.paidamount))}</TableCell>
                                                <TableCell align="right">{formatReadableAmount(amt(item.adjusted, item.millamount, item.Quantal, item.PartyDispQty, item.paidamount))}</TableCell>
                                                <TableCell align="right">{formatReadableAmount(bqty(item.adjusted, item.millamount, item.Quantal, item.PartyDispQty, item.paidamount))}</TableCell>
                                                <TableCell align="center">{item.Lifting_DateConverted}</TableCell>
                                            </TableRow>
                                        ))}

                                        <TableRow>
                                            <TableCell colSpan={3} align="right" sx={{ fontWeight: 'bold' }}>{formatReadableAmount(subGroupData.totalQty)}</TableCell>
                                            <TableCell align="right" sx={{ fontWeight: 'bold' }}>{formatReadableAmount(subGroupData.PartyDispatchQty)}</TableCell>
                                            <TableCell align="right" sx={{ fontWeight: 'bold' }}>{formatReadableAmount(subGroupData.frmmilladjamt)}</TableCell>
                                            ...
                                            <TableCell sx={{ fontWeight: 'bold' }}>Paid Qty</TableCell>
                                ...
                                <TableRow>
                                    <TableCell colSpan={12} align="right" sx={{ fontWeight: 'bold', color: 'red', backgroundColor: '#fff0f0' }}>
