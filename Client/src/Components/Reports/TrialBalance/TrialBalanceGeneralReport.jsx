...
// Import required assets for PDF
...
// Column Definitions
const COLUMNS = [
    { label: 'Account Code', key: 'AC_CODE', width: '15%' },
    { label: 'Account Name', key: 'Ac_Name_E', width: '40%' },
    { label: 'City', key: 'CityName', width: '15%' },
...
const TrialBalanceReport = () => {
    ...
    const handleGeneratePDF = () => {
        ...
        Object.entries(groupedReportData).forEach(([groupName, data]) => {
            // Group Header
            pdfRows.push([{ content: groupName, colSpan: 5, styles: { fillColor: [210, 233, 198], fontStyle: 'bold' } }]);
            ...
            data.items.forEach(item => {
                pdfRows.push([
                    ...
                    item.Ac_Name_E,
                    item.CityName,
                    formatReadableAmount(item.Balance > 0 ? item.Balance : 0),
                    formatReadableAmount(item.Balance < 0 ? Math.abs(item.Balance) : 0)
                ]);
            });

            // Group Footer
            pdfRows.push([
                { content: `Group Total:`, colSpan: 3, styles: { halign: 'right', fontStyle: 'bold' } },
                formatReadableAmount(Math.abs(data.groupdebitamt)),
                formatReadableAmount(Math.abs(data.groupcreditamt))
            ]);

            totalDebit += Math.abs(data.groupdebitamt);
            totalCredit += Math.abs(data.groupcreditamt);
        });
        ...
        generateReportPDF({
            ...
            columnWidths: [25, 80, 35, 25, 25],
            rows: pdfRows,
            footerRow: ['', 'NET TOTAL', '', { content: formatReadableAmount(totalDebit), styles: { halign: 'right' } }, { content: formatReadableAmount(totalCredit), styles: { halign: 'right' } }],
            numericCols: [3, 4],
    ...
    return (
        <div style={{ marginTop: "-80px" }}>
            ...
            {pdfPreview && <PdfPreview pdfData={pdfPreview} label="trialBalance" />}
