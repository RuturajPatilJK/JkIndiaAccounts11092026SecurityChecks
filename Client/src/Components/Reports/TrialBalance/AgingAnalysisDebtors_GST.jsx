...
const AgingAnalysisDetors_GSTReport = () => {
  ...
  const API_URL = `${apikey}/AgingAnalysis-Report-Debtors_GST`;
  ...
  const groupReportData = (data) => {
    ...
    if (sortConfig.key !== null) {
      groupedArr.sort((a, b) => {
        let valA, valB;
        if (['PAN','Ac_Name_E'].includes(sortConfig.key)) {
    ...
    win.document.write(`
      <html>
        ...
        <body>
          ...
          <div style="text-align:center;">Aging Analysis - Debtors PAN wise</div>
  ...
  const generatePdf = () => {
    ...
    doc.setFontSize(10);
    doc.text(`Aging Analysis - Debtors PAN wise | As On: ${toDate}`, 14, 16);
  ...
  const handleExportToExcel = () => {
    ...
    wsData.push([`Aging Analysis - Debtors PAN wise (As On: ${toDate})`]);
    wsData.push([]);
    wsData.push(["PAN","A/C Name", ...BUCKETS.map(b => b.label), "Misc","Balance"]);
    ...
    XLSX.writeFile(wb, "Aging_Debtors_PANwise_Sorted.xlsx");
  };
      ...
      <div className="d-flex justify-content-between align-items-center">
        <div style={{ flex: 1, textAlign: "center", marginLeft: "280px" }}>
          <Typography variant="h6" style={{ fontSize: "20px", fontWeight: "bold" }}>
            Aging Analysis - Debtors PANwise
          </Typography>
        </div>
          ...
          <table className="table table-striped table-bordered mt-4" id="reportTable" style={{ marginBottom: "60px", width: "100%" }}>
            <thead className="table-light" style={{ position: "sticky", top: 0, backgroundColor: "#fff", zIndex: 1 }}>
              <tr style={{ cursor: 'pointer' }}>
                ...
                <th onClick={() => requestSort('Gst_No')}>
                 PAN No <span style={{ color: '#aaa' }}>{renderSortIcon('PAN')}</span>
                </th>
                <th onClick={() => requestSort('Ac_Name_E')}>
                A/c Name <span style={{ color: '#aaa' }}>{renderSortIcon('Ac_Name_E')}</span>
                </th>
              ...
              {sortedGroupedData.map(({ key, items }) => (
                <React.Fragment key={key}>
                  {items.map((item, index) => (
                    <tr key={index}>
                      <td onClick={() => handleRowClick(item.PAN)} style={{ cursor: "pointer", verticalAlign: "top", backgroundColor: "#D0E9C6", fontStyle: "italic" }}>{item.PAN}</td>
...
export default AgingAnalysisDetors_GSTReport;
