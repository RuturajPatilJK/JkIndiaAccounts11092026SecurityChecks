...
const TrialBalanceDetailReportGSTWise = () => {
    ...
    const fetchGLedgerReport = async () => {
      try {
        ...
        const response = await axios.get(
          `${process.env.REACT_APP_API}/TrialBalanceDetailGSTwise-Report`,
          {
            params: {
              from_date: fromDate,
              to_date: toDate,
  ...
  return (
    <div className="ledger-report-container">
      ...
      <div id="printArea">
        ...
        {ledgerData.length > 0 && (
          <>
            <table id="reportTable" style={{ marginBottom: "100px" }}>
              <thead>
                <tr>
                  ...
                  <React.Fragment key={groupName}>
                    ...
                    {items.map((item, index) => (
                      <tr key={`${groupName}-${index}`} onClick={() => handleRowClick(item.DOC_NO, item.TRAN_TYPE)}>
                        ...
                        <td>{item.Gst_No}</td>
                        <td style={{ textAlign: "right" }}>{formatReadableAmount(item.Op_Debit)}</td>
                        <td style={{ textAlign: "right" }}>{formatReadableAmount(item.Op_Credit)}</td>
                        <td style={{ textAlign: "right" }}>{formatReadableAmount(item.Tran_Debit)}</td>
                        <td style={{ textAlign: "right" }}>{formatReadableAmount(item.Tran_Credit)}</td>
                        <td style={{ textAlign: "right" }}>{formatReadableAmount(item.Closing_Debit)}</td>
                        <td style={{ textAlign: "right" }}>{formatReadableAmount(item.Closing_Credit)}</td>
                        <td>{item.drcr}</td>
                      </tr>
                    ))}
                  </React.Fragment>
              ...
              <tfoot>
                <tr>
                  ...
                  <td style={{ textAlign: "right" }}><strong>{formatReadableAmount(totals.Op_Debit)}</strong></td>
                  <td style={{ textAlign: "right" }}><strong>{formatReadableAmount(totals.Op_Credit)}</strong></td>
                  <td style={{ textAlign: "right" }}><strong>{formatReadableAmount(totals.Tran_Debit)}</strong></td>
                  <td style={{ textAlign: "right" }}><strong>{formatReadableAmount(totals.Tran_Credit)}</strong></td>
                  <td style={{ textAlign: "right" }}><strong>{formatReadableAmount(totals.Closing_Debit)}</strong></td>
                  <td style={{ textAlign: "right" }}><strong>{formatReadableAmount(totals.Closing_Credit)}</strong></td>
                </tr>
              </tfoot>
            </table>
...
export default TrialBalanceDetailReportGSTWise;
