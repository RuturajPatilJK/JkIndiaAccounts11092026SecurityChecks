...
function ShetkariPurchaseBillUtility() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            setIsLoading(true);
            try {
                const apiUrl = `${API_URL}/getdata-shetkaripurchase?Company_Code=${companyCode}&Year_Code=${Year_Code}&tran_type=${tranType}`;
                const response = await axios.get(apiUrl);
                const data = response.data?.shetkaripurchase_Head || [];
                setFetchedData(data);
            } catch (error) {
    ...
    const paginatedData = useMemo(() => {
        const filtered = fetchedData.filter((post) => {
            ...
            return (
                ...
                String(post.Ac_Name_E || "").toLowerCase().includes(searchTermLower) ||
                String(post.Doc_Date || "").toLowerCase().includes(searchTermLower) ||
                String(post.Truck_No || "").toLowerCase().includes(searchTermLower) 
    ...
    const handleClick = () => {
        navigate("/shetkaripurchase", { state: { tranType, permissionsData } });
    };

    const handleRowClick = (purchaseid) => {
  
        const selectedRecord = fetchedData.find((record) => record.purchaseid === purchaseid);
        navigate("/shetkaripurchase", { state: { selectedRecord, permissionsData } });
    };
                            ...
                            <Table stickyHeader>
                                <TableHead>
                                    <TableRow sx={{ backgroundColor: "#f4f4f4" }} >
                                        ...
                                        <TableCell style={styles.tableHeaderCell}>Truck No</TableCell>
                                ...
                                <TableBody>
                                    {paginatedData.paginatedPosts.map((post) => (
                                        <TableRow
                                            key={post.purchaseid}
                                            onDoubleClick={() => handleRowClick(post.purchaseid)}
                                            sx={{
                                                cursor: "pointer",
                                        ...
                                        >
                                            ...
                                            <TableCell style={{ whiteSpace: "nowrap" }}>{post.Ac_Name_E || ""}</TableCell>
...
export default ShetkariPurchaseBillUtility;;
