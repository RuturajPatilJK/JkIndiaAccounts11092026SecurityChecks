...
const ReturnPurchaseReport = ({ doc_no, disabledFeild }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(
        `${API_URL}/generating_return_purchase_report?Company_Code=${companyCode}&Year_Code=${Year_Code}&doc_no=${doc_no}`
      );
      if (!response.ok) {
  ...
  const generatePdf = async (data) => {
    ...
    let qrCodeData = `
      ...
      GSTIN of Buyer: ${safe(allData.billgstno)}
      Document No: PR${safe(allData.year)}-${safe(allData.doc_no)}
      Document Type: Tax Invoice
      Date Of Creation Of Invoice: ${safe(allData.dt)}
    ...
    logoImg.onload = () => {
      ...
      const tableData = [
        ["Reverse Charge", "No"],
        ["Invoice No:", `PR${safe(allData.year)}-${safe(allData.doc_no)}`],
        ["Invoice Date:", safe(allData.dt)],
        ["State:", safe(allData.companyStateName)],
        ["State Code:", safe(allData.companyGSTStateCode)],
        ["Buyer,"],
        [safe(allData.billtoname)],
        [`${safe(allData.billaddress)}, ${safe(allData.billtostatename)}`],
        ["Bill To:"],
        ["City:", safe(allData.billcityname)],
        ["State:", safe(allData.billtostatename)],
        ["Gst No:", safe(allData.billgstno)],
        ["State Code:", safe(allData.billstatecode)],
        ["PAN No:", safe(allData.billcompanypan)],
      ];
  
      const buyerData = [
        ["Our GST No:", safe(allData.GST)],
        ["Transport Mode:", "Road"],
        ["Date Of Supply:", safe(allData.dt)],
        ["Place Of Supply:", safe(allData.To_State)],
        ["Consigned To,"],
        [safe(allData.UnitName)],
        [safe(allData.UnitAddress)],
        ["Ship To,"],
        ["City:", safe(allData.UnitCity)],
        ["State:", safe(allData.UnitState)],
        ["Gst No:", safe(allData.UnitGSTStateCode)],
        ["State Code:", safe(allData.UnitGST)],
        ["PAN No:", safe(allData.UnitPan)],
      ];
      ...
      pdf.text(`Mill Name : ${safe(allData.Mill_Name)}`, 10, 135);
      pdf.text(`Dispatched From : ${safe(allData.From_Place)}`, 10, 140);
      pdf.text(`Lorry No : ${safe(allData.lorry)}`, 80, 140);
      pdf.text(`To : ${safe(allData.To_Place)}`, 130, 140);
      ...
      const summaryData = [
        ["SubTotal:", "", safe(allData.Sub_Total)],
        ...
        ["Freight:", "", safe(allData.LESS_FRT_RATE)],
        ["Bank Commission", "", safe(allData.Bank_Commission)],
        ["Other Expense:", "", safe(allData.Other_Expenses)],
...
export default ReturnPurchaseReport;