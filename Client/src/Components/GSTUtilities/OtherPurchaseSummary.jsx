...
// const OtherPurchaseSummary = ({ fromDate, toDate, companyCode, yearCode,accode }) => {
...
const SCREEN_COLUMNS = [
    ...
    { label: 'Bill No',         key: 'BillNo',         numeric: false, isTotal: false },
    { label: 'TDS Amount',      key: 'TDSAmount',      numeric: true,  isTotal: true },
    { label: 'Narration',       key: 'Narration',      numeric: false, isTotal: false },
];
...
const PRINT_COLUMNS = [
    ...
    { label: 'TDS Amount',      key: 'TDSAmount',      printWidth: 25, numeric: true, isTotal: true },
    { label: 'Bill Amount',    key: 'Bill_Amount',    printWidth: 35, numeric: true, isTotal: true },
];
...
const OtherPurchaseSummary = ({ fromDate, toDate, companyCode, yearCode, accode }) => {
    ...
    // Listener for the dashboard window print button
    useEffect(() => {
        const handleMessage = (event) => {
            if (event.data === 'TRIGGER_OTHER_PURCHASE_PRINT') {
                handleGeneratePDF();
            }
        };
...
<html><head><title>Other Purchase Summary</title>
<script src="https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.18.5/xlsx.full.min.js"></script>
<style>
...
<div class="toolbar">
  ...
  <div style="display:flex;gap:8px">
    ...
    <button class="btn btn-green" onclick="exportXlsx()">Export</button>
  </div>
</div>
...
function render(data){
  ...
  document.getElementById('cards').innerHTML = ['TaxableAmount','CGST','SGST','IGST','Bill_Amount','TDSAmount'].map(k=>\`
    <div class="card">
...
function printPremiumPDF(){ window.opener.postMessage('TRIGGER_OTHER_PURCHASE_PRINT', '*'); }
...
function exportXlsx(){
  ...
  const wsData = [['Other Purchase Summary'], ['Period: ${FormaDateBalanceSheet(fromDate)} to ${FormaDateBalanceSheet(toDate)}'], [], headers, ...dataRows, [], totalRow];
    ...
    return (
        <div style={{textAlign: 'center' }}>
            <button 
                ...
                disabled={loading}
            >
                {loading ? <CircularProgress size={24} color="inherit" /> : "Other Purchase Summary"}
            </button>

            {error && <Alert severity="error" sx={{ mt: 2 }}>{error}</Alert>}
            {pdfPreview && <PdfPreview pdfData={pdfPreview} label="GSTOtherPurchaseSummary" />}
            ...
            {(loading || isPrinting) && (
                <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(255,255,255,0.8)', zIndex: 9999, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
                    ...
                    <Typography sx={{ mt: 2, fontWeight: 700, color: '#1a237e' }}>
                        {isPrinting ? 'Generating Professional PDF...' : 'Fetching Report Data...'}
                    </Typography>
                </div>
            )}
        </div>
