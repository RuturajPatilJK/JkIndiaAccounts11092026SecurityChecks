...
const SCREEN_COLUMNS = [
    { label: 'GSTIN of Recipient', key: 'GSTIN/UIN of Recipient', numeric: false, isTotal: false },
    { label: 'Invoice No', key: 'Invoice Number', numeric: false, isTotal: false },
    { label: 'Date', key: 'Invoice date', numeric: false, isTotal: false },
    { label: 'Invoice Value', key: 'Invoice Value', numeric: true, isTotal: true },
    { label: 'POS', key: 'Place Of Supply', numeric: false, isTotal: false },
    { label: 'Rev Charge', key: 'Reverse Charge', numeric: false, isTotal: false },
    { label: 'Type', key: 'Invoice Type', numeric: false, isTotal: false },
    { label: 'Rate', key: 'Rate', numeric: true, isTotal: false },
    { label: 'Taxable Value', key: 'Taxable Value', numeric: true, isTotal: true },
    { label: 'Cess', key: 'Cess Amount', numeric: true, isTotal: true },
];

const PRINT_COLUMNS = [
    { label: 'GSTIN', key: 'GSTIN/UIN of Recipient', printWidth: 40 },
    { label: 'Inv No', key: 'Invoice Number', printWidth: 30 },
    { label: 'Date', key: 'Invoice date', printWidth: 25 },
    { label: 'Inv Value', key: 'Invoice Value', printWidth: 35, numeric: true, isTotal: true },
    { label: 'POS', key: 'Place Of Supply', printWidth: 20 },
    { label: 'Type', key: 'Invoice Type', printWidth: 30 },
    { label: 'Rate', key: 'Rate', printWidth: 15, numeric: true },
    { label: 'Taxable Val', key: 'Taxable Value', printWidth: 35, numeric: true, isTotal: true },
];
...
const CreateB2BFile = ({ fromDate, toDate, companyCode, yearCode }) => {
    ...
    useEffect(() => {
        const handleMessage = (event) => {
            if (event.data === 'TRIGGER_B2B_PRINT') handleGeneratePDF();
        };
    ...
    const fetchHSNWiseSummary = async () => {
        try {
            ...
            if (!response.data || response.data.length === 0) {
                Swal.fire({ icon: 'error', title: 'Data Not Found!', text: 'No B2B records found.' });
                return;
            }
            setData(response.data);
    ...
    const handleGeneratePDF = () => {
        ...
        const footerRow = PRINT_COLUMNS.map(col => {
            ...
            return col.label === 'GSTIN' ? { content: 'TOTAL', styles: { fontStyle: 'bold', fillColor: [255, 249, 196] } } : '';
        });

        generateReportPDF({
            title: `B2B File Report`,
...
<html><head><title>B2B File Dashboard</title>
<script src="https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.18.5/xlsx.full.min.js"></script>
<style>
...
<div class="toolbar">
  ...
  <div style="display:flex;gap:8px">
    <button class="btn btn-red" onclick="window.opener.postMessage('TRIGGER_B2B_PRINT', '*')">Print PDF</button>
...
function exportXlsx(){
  ...
  const wsData = [['B2B File Report'],['${FormaDateBalanceSheet(fromDate)} to ${FormaDateBalanceSheet(toDate)}'],[],headers,...dataRows];
  const ws = XLSX.utils.aoa_to_sheet(wsData);
  XLSX.utils.book_append_sheet(wb, ws, 'B2BReport');
  XLSX.writeFile(wb, 'CreateB2BFile.xlsx');
}
render(RAW);
</script></body></html>`);
    ...
    return (
        <div style={{ padding: '5px', textAlign: 'center' }}>
            <button style={{
                ...
                {loading ? <CircularProgress size={24} color="inherit" /> : "Create B2B File"}
            </button>
            {error && <Alert severity="error" sx={{ mt: 2 }}>{error}</Alert>}
            {pdfPreview && <PdfPreview pdfData={pdfPreview} label="GSTCreateB2B" />}
            {(loading || isPrinting) && (
                <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(255,255,255,0.8)', zIndex: 9999, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
                    ...
                    <Typography sx={{ mt: 2, fontWeight: 700, color: '#1a237e' }}>
                        {isPrinting ? 'Preparing B2B PDF...' : 'Fetching Data...'}
                    </Typography>
                </div>
            )}
        </div>
