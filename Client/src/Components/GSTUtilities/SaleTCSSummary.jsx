...
//             link.setAttribute("download", "SaleTCSSummary.xlsx");
...
const SCREEN_COLUMNS = [
    ...
    { label: 'TCS Amount', key: 'TCS', numeric: true, isTotal: true },
];

const PRINT_COLUMNS = [
    { label: 'Invoice No', key: 'InvoiceNo', printWidth: 25 },
    { label: 'Date', key: 'date', printWidth: 25 },
    { label: 'Party Name', key: 'Name Of Party', printWidth: 80 },
    ...
    { label: 'TCS Amount', key: 'TCS', printWidth: 20, numeric: true, isTotal: true },
...
const SaleTCSSummary = ({ fromDate, toDate, companyCode, yearCode, Tran_type, accode }) => {
    ...
    useEffect(() => {
        const handleMessage = (event) => {
            if (event.data === 'TRIGGER_SALE_TCS_PRINT') {
                handleGeneratePDF();
            }
        };
    ...
    const grandTotals = useMemo(() => {
        const t = { Taxable_Amt: 0, CGST: 0, SGST: 0, IGST: 0, Bill_Amt: 0, TCS: 0 };
        data.forEach(row => {
    ...
    const fetchSaleTCSSummary = async () => {
        try {
            ...
            if (!response.data || response.data.length === 0) {
                Swal.fire({ icon: 'error', title: 'Data Not Found!', text: 'No Sale TCS records found.' });
                return;
            }
...
<html><head><title>Sale TCS Summary</title>
<script src="https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.18.5/xlsx.full.min.js"></script>
<style>
...
<h2>Sale TCS Summary</h2>
<div class="sub">${FormaDateBalanceSheet(fromDate)} to ${FormaDateBalanceSheet(toDate)}</div>
<div class="cards" id="cards"></div>
<div class="toolbar">
  <input class="search" id="searchBox" placeholder="Search Party, PAN, Invoice..." oninput="filterTable()">
...
function render(data){
  ...
  document.getElementById('cards').innerHTML = ['Taxable_Amt','CGST','SGST','IGST','Bill_Amt','TCS'].map(k=>\`
    <div class="card">
...
function printReport(){ window.opener.postMessage('TRIGGER_SALE_TCS_PRINT', '*'); }

function exportXlsx(){
  const wb = XLSX.utils.book_new();
  const headers = COLS.map(c => c.label);
  const dataRows = filtered.map(row => COLS.map(c => c.numeric ? parseFloat(row[c.key] || 0) : (row[c.key] || '')));
  const ws = XLSX.utils.aoa_to_sheet([['Sale TCS Summary'], ['Period: ${fromDate} to ${toDate}'], [], headers, ...dataRows]);
  XLSX.utils.book_append_sheet(wb, ws, 'SaleTCS_Summary');
  XLSX.writeFile(wb, 'SaleTCS_Summary.xlsx');
}
render(RAW);
</script></body></html>`);
        newWindow.document.close();
    ...
    return (
        <Box sx={{ textAlign: 'center', marginTop: '5px' }}>
            ...
            >
                {loading ? <CircularProgress size={24} color="inherit" /> : 'Sale TCS Summary'}
            </Button>

            {error && <Alert severity="error" sx={{ mt: 2, maxWidth: '450px', mx: 'auto' }}>{error}</Alert>}
            {pdfPreview && <PdfPreview pdfData={pdfPreview} label="GSTSaleTCSSummary" />}
