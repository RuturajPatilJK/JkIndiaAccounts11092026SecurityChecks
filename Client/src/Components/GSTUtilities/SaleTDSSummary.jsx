...
//                 // Set number format for each of the numeric columns
...
//                     <script>${exportToXlsxScript}</script>
//                                 <style>
...
const SaleTDSSummary = ({ fromDate, toDate, companyCode, yearCode, Tran_type, accode }) => {
    ...
    useEffect(() => {
        const handleMessage = (event) => {
            if (event.data === 'TRIGGER_SALETDS_PRINT') {
                handleGeneratePDF();
            }
        };
    ...
    const grandTotals = useMemo(() => {
        const t = { Taxable_Amt: 0, CGST: 0, SGST: 0, IGST: 0, Bill_Amt: 0, TDS: 0 };
        data.forEach(row => {
    ...
    const fetchSaleTDSSummary = async () => {
        try {
            ...
            if (!response.data || response.data.length === 0) {
                Swal.fire({ icon: 'error', title: 'Data Not Found!', text: 'No Sale TDS data found.' });
                return;
            }
...
<html><head><title>Sale TDS Summary</title>
<script src="https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.18.5/xlsx.full.min.js"></script>
<style>
...
function render(data){
  ...
  document.getElementById('cards').innerHTML = ['Taxable_Amt','CGST','SGST','IGST','Bill_Amt','TDS'].map(k=>\`
    <div class="card">
...
function printReport(){ window.opener.postMessage('TRIGGER_SALETDS_PRINT', '*'); }

function exportXlsx(){
  const wb = XLSX.utils.book_new();
  const headers = COLS.map(c => c.label);
  const dataRows = filtered.map(row => COLS.map(c => c.numeric ? parseFloat(row[c.key] || 0) : (row[c.key] || '')));
  const ws = XLSX.utils.aoa_to_sheet([['Sale TDS Summary'], ['Period: ${fromDate} to ${toDate}'], [], headers, ...dataRows]);
  XLSX.utils.book_append_sheet(wb, ws, 'SaleTDSSummary');
  XLSX.writeFile(wb, 'SaleTDSSummary.xlsx');
}
render(RAW);
</script></body></html>`);
        newWindow.document.close();
    ...
    return (
        <div style={{ textAlign: 'center' }}>
            <button
                ...
                disabled={loading}
            >
                {loading ? <CircularProgress size={24} color="inherit" /> : "Sale TDS Summary"}
            </button>

            {error && <Alert severity="error" sx={{ mt: 2, maxWidth: '450px', mx: 'auto' }}>{error}</Alert>}
            {pdfPreview && <PdfPreview pdfData={pdfPreview} label="SaleTDSSummary" />}
