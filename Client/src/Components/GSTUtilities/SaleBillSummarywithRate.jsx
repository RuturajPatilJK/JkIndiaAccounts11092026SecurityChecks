...
const SaleBillSummarywithRate = ({ fromDate, toDate, companyCode, yearCode, accode }) => {
    ...
    const fetchSaleBillSummary = async () => {
        try {
            ...
            setError('');
            setPdfPreview(null);
            const response = await axios.get(`${API_URL}/salebill-summarywithmillrate`, {
    ...
    const handleGeneratePDF = () => {
        ...
        generateReportPDF({
            title: 'Sale Bill Summary With Mill Rate Report',
...
<html><head><title>Sale Bill Summary With Mill Rate</title>
...
<h2>Sale Bill Summary With Mill Rate</h2>
...
function exportXlsx(){
  ...
  XLSX.writeFile(wb, 'SaleBillSummaryWithMillRate.xlsx');
}
render(RAW);
</script></body></html>`);
    ...
    return (
        <div style={{ textAlign: 'center', marginTop: '5px' }}>
            ...
            >
                {loading ? <CircularProgress size={24} color="inherit" /> : "Mill Rate Wise Sale"}
            </button>

            {error && <Alert severity="error" sx={{ mt: 2, maxWidth: '400px', mx: 'auto' }}>{error}</Alert>}
            {pdfPreview && <PdfPreview pdfData={pdfPreview} label="GSTSaleBillSummarywithrate" />}
...
export default SaleBillSummarywithRate;