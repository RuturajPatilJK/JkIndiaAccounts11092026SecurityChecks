...
const CustomizeSBReport = ({ saleId, ewayBillNo, handleSaleBilReportClose, hidePreview, onReady, signWithDSC = false }) => {
   ...
   const fetchDataAndGeneratePdf = async () => {
    try {
      const response = await fetch(
        `${API_URL}/generating_saleBill_report_onlineportal?Company_Code=${companyCode}&Year_Code=${Year_Code}&saleid=${saleId}`
      );
      if (!response.ok) {
      ...
      await generatePdf(data.all_data);
    } catch (error) {
      console.error("Error fetching data:", error);
      alert("Failed to generate the sale bill preview. Please try again.");
      if (handleSaleBilReportClose) {
        handleSaleBilReportClose(); 
      }
    }
  };
  ...
  // Mirrors Outward/SaleBill/CustomizeSBReport.jsx's signPdfWithDSC.
...
const shouldUseImage =
 ...
 pdf.setFontSize(9);
      ...
      // backend can stamp the real digital signature in the gap BETWEEN
      // "For. <Company>" and "Authorised Signatory" - mirrors the same logic
      // in Outward/SaleBill/CustomizeSBReport.jsx.
      let dscBox = null;
      let authorisedSignatoryY = y + 28;
      if (signWithDSC) {
        ...
        const gapTopMm = signY + 2;
        authorisedSignatoryY = Math.min(gapTopMm + 28, footerTopMm - 10);
        const boxTopMm = gapTopMm;
        const boxBottomMm = authorisedSignatoryY - 2;
        dscBox = {
      ...
      pdf.text("Authorised Signatory", 168, authorisedSignatoryY);
      ...
      const unsignedBlob = pdf.output("blob");
      const finalBlob = signWithDSC ? await signPdfWithDSC(unsignedBlob, dscBox) : unsignedBlob;
