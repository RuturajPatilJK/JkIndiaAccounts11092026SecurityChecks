...
const OtherCustomizeSBReport = ({ doc_no,tran_type, disabledFeild }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(
        `${API_URL}/generating_othersaleBill_report?Company_Code=${companyCode}&Year_Code=${Year_Code}&doc_no=${doc_no}&Tran_Type=${tran_type}`
      );
      if (!response.ok) {
...
const shouldUseImage =
 ...
 pdf.setFontSize(9);
      ...
      const fieldPairs = [
        [
          `Invoice No. :  SO${allData.year}-${allData.doc_no}`,
    ...
    //   pdf.text(String(allData.season ?? ""), 120, y + 9);
      ...
      pdf.text(`${formattedQty} MT`, labelX, y + 9);
...
if (rate !== null && rate !== undefined && rate !== "") {
  ...
  data: {
    ...allData,
    sale_rate: rate, 
   Doc_No: `SO${allData?.year || ""}-${allData?.doc_no || ""}`,
...
export default OtherCustomizeSBReport;