 ...
 pdf.setFontSize(9);
      ...
      const fieldPairs = [
        ...
        [
          ...
          [ `PO Details: ${allData.pono}`,"", "Reverse Charge", "No"],
      ];
      ...
      pdf.text(`${formattedQty}`, labelX, y + 9);
      pdf.text(formattedRate, rateX, y + 9, { align: "right" });
      pdf.text(formatReadableAmount(allData.TaxableAmount), amountX, y + 9, { align: "right" });

      y += 8;
      pdf.setFont("Signika-Regular");

      const wrappedMillName = pdf.splitTextToSize(String(allData.millname ?? "-"), 90);
      // pdf.text("Narration:", 12, y + 11);
      pdf.text(allData.SBNarration, 12, y + 11);
    ...
    parseFloat(allData.freight) !== 0
      ...
      summaryFields.forEach(([label, rate, amount]) => {
        ...
        //   pdf.text(`${formatReadableAmount(rate)}/Qntl`, 165, y + 3, { align: "center" });
        // }
...
// --- Start of Updated Paragraphs Code ---
y += 35; 

// Set to Bold as requested
pdf.setFont("Signika-Regular");
pdf.setFontSize(8);
pdf.setTextColor(0, 0, 0); 

const dummyText = [
  "\"We hereby certify that goods mentioned in this invoice are warranted to be of the nature and quality which these purport to be.\"\nCertify that the particulars given above are true and correct and the amount indicated represent the price actually charged and that there is no flow of additional consideration directly or indirectly from the buyer."
];

dummyText.forEach((paragraph) => {
  const wrappedParagraph = pdf.splitTextToSize(paragraph, 185); 
  pdf.text(wrappedParagraph, 12, y);
  
  y += (wrappedParagraph.length * 4) + 5; 
});
...
/>
      ...
      <PrintButton disabledFeild={disabledFeild} fetchData={fetchData} label={"Corporate"} />
    </div>
  );
};
