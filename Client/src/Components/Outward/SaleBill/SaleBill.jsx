...
const SaleBill = () => {
  ...
  const fetchLastRecord = () => {
    fetch(
      `${API_URL}/get-next-doc-no?Company_Code=${companyCode}&Year_Code=${Year_Code}&Tran_Type=${formData.Tran_Type}`
    )
      .then((response) => {
        if (!response.ok) {
  ...
  const handleSaveOrUpdate = async () => {
    ...
    try {
      if (isEditMode) {
        ...
        navigate(`/sale-bill?navigatedRecord=${formData.doc_no}`);
      } else {
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await axios.get(
        `${API_URL}/SaleBillByid?saleid=${newSaleid}&Company_Code=${companyCode}&Year_Code=${Year_Code}`
      );
      ...
      if (result.isConfirmed) {
        ...
        const deleteApiUrl = `${API_URL}/delete_data_by_saleid?saleid=${newSaleid}&Company_Code=${companyCode}&doc_no=${formData.doc_no}&Year_Code=${Year_Code}&User_Id=${User_Id}&Tran_Type=${formData.Tran_Type}`;
  ...
  const handleCancel = async () => {
    ...
    try {
      const response = await axios.get(
        `${API_URL}/get-lastSaleBill-navigation?Company_Code=${companyCode}&Year_Code=${Year_Code}&Tran_Type=${formData.Tran_Type}`
      );
  ...
  const handleFirstButtonClick = async () => {
    try {
      const response = await axios.get(
        `${API_URL}/get-firstSaleBill-navigation?Company_Code=${companyCode}&Year_Code=${Year_Code}&Tran_Type=${formData.Tran_Type}`
      );
      if (response.status === 200) {
        const data = response.data;
  ...
  const handleNextButtonClick = async () => {
    try {
      const response = await axios.get(
        `${API_URL}/get-nextSaleBill-navigation?Company_Code=${companyCode}&Year_Code=${Year_Code}&currentDocNo=${formData.doc_no}&Tran_Type=${formData.Tran_Type}`
      );
      if (response.status === 200) {
        const data = response.data;
  ...
  const handlePreviousButtonClick = async () => {
    try {
      const response = await axios.get(
        `${API_URL}/get-previousSaleBill-navigation?Company_Code=${companyCode}&Year_Code=${Year_Code}&currentDocNo=${formData.doc_no}&Tran_Type=${formData.Tran_Type}`
      );
  ...
  const handleNavigateRecord = async () => {
    ...
    try {
      const response = await axios.get(
        `${API_URL}/SaleBillByDocNo?doc_no=${navigatedRecord}&Company_Code=${companyCode}&Year_Code=${Year_Code}&Tran_Type=${formData.Tran_Type}`
      );
      if (response.status === 200) {
        const data = response.data;
  ...
  const handleKeyDown = async (event) => {
    if (event.key === "Tab") {
      ...
      try {
        const response = await axios.get(
          `${API_URL}/SaleBillByDocNo?doc_no=${changeNoValue}&Company_Code=${companyCode}&Year_Code=${Year_Code}&Tran_Type=${formData.Tran_Type}`
        );
        const data = response.data;
        // newSaleid = data.last_head_data.saleid;
        partyName = data.last_details_data[0].partyname;
  ...
  const tdsCalcInfoRef = useRef({ balancelimit: 0, tdsApplicable: 'N', saleTdsRate: 0, tcsRate: 0, priorAmt: 0 });

  const computeSaleTdsTcsRates = (currentTaxableAmount) => {
    const info = tdsCalcInfoRef.current;
    const totalAmount = (info.priorAmt || 0) + (parseFloat(currentTaxableAmount) || 0);
    if (totalAmount >= (info.balancelimit || 0)) {
      if (info.tdsApplicable === 'Y') {
        return { TDS_Rate: info.saleTdsRate || 0, TCS_Rate: 0 };
      }
  ...
  const AmountCalculation = async (name, input, formData) => {
    ...
    const currentTaxableAmount = parseFloat(formData.TaxableAmount) || 0;
    const { TDS_Rate, TCS_Rate } = computeSaleTdsTcsRates(currentTaxableAmount);
    ...
    const updatedFormData = {
      ...
      TDS_Rate,
      TCS_Rate,
      TDS_Amt: ((currentTaxableAmount * TDS_Rate) / 100).toFixed(2),
    ...
    <>
      ...
      <ActionButtonGroup
        ...
        component={<div style={{ display: 'flex' }} >
          ...
          <CustomizeSBReport doc_no={formData.doc_no} disabledFeild={!addOneButtonEnabled} />
