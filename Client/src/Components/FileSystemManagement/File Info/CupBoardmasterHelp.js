...
const GSTStateMasterHelp = ({ onAcCodeClick, name, newCupBoardCode, newUserName,disabledFeild,tabIndexHelp }) => {
    const [showModal, setShowModal] = useState(false);
    const [popupContent, setPopupContent] = useState([]);
    const [enteredAcCode, setEnteredAcCode] = useState(newCupBoardCode || "");
    const [enteredAcName, setEnteredAcName] = useState(newUserName || "");
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`${API_URL}/getallCupBoard`);
                setPopupContent(response.data.alldata || []);
                if (newCupBoardCode) {
                    const foundItem = response.data.alldata.find(item => item.Cupboard_Code.toString() === newCupBoardCode.toString());
                    if (foundItem) {
                        setEnteredAcCode(foundItem.Cupboard_Code);
                        setEnteredAcName(foundItem.Cupboard_Name);
                    }
                }
            } catch (error) {
    ...
    useEffect(() => {
        setEnteredAcCode(newCupBoardCode || "");
        setEnteredAcName(newUserName || "");
    }, [newCupBoardCode, newUserName]);


    const filteredData = useMemo(() => {
        return popupContent.filter((item) =>
            item.Cupboard_Name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
            String(item.Cupboard_Code)?.toLowerCase().includes(searchTerm.toLowerCase())
        );
    ...
    const handleAcCodeChange = (event) => {
        ...
        const matchingItem = popupContent.find((item) => item.Cupboard_Code.toString() === value);
        if (matchingItem) {
            setEnteredAcName(matchingItem.Cupboard_Name);
            onAcCodeClick?.(matchingItem.Cupboard_Code, matchingItem.Cupboard_Name);
        }
    };
