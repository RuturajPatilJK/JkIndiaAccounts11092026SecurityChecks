...
import { fetchOtherPurchase, otherPurchaseSelectors } from "../../../store/otherPurchaseSlice";
import useOtherPurchaseSocket from "../../../hooks/useOtherPurchaseSocket";
...
function OtherPurchaseLiveTable({
    title,
    columns,
    rowKey,
    addUrl,
    ...
    const fetchedData = useSelector(otherPurchaseSelectors.selectAll);
    const status = useSelector((state) => state.otherPurchase.status);
    useOtherPurchaseSocket(companyCode, Year_Code);
    ...
    // Fetch permissions, then load data into Redux
    useEffect(() => {
        const checkPermissions = async () => {
            try {
                ...
                if (response.data?.UserDetails?.canView === 'Y') {
                    setCanView(true);
                    // Fetch once per store lifetime — the socket (useOtherPurchaseSocket)
                    // keeps the cached data live after that, so revisiting this page
                    // doesn't need a fresh network round trip.
                    if (status === 'idle') {
                        dispatch(fetchOtherPurchase({ companyCode, yearCode: Year_Code }));
                    }
                } else {
                    setCanView(false);
                }
...
export default OtherPurchaseLiveTable;
