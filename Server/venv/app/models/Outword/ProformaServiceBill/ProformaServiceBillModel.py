...
class ProformaServiceBillHead(db.Model):
    __tablename__ = 'nt_1_proformaservicebillhead'
    ...
    eventCode = Column(Integer,nullable=True)
    eventId = Column(Integer,nullable=True)
    Discount_Rate = Column(Numeric(18, 2), nullable=True)
    Discount_Amount = Column(Numeric(18, 2), nullable=True)
    Taxable_Amount = Column(Numeric(18, 2), nullable=True)
    

    details = db.relationship('ProformaServiceBillDetail', backref='proformaservicebillhead', lazy=True)

class ProformaServiceBillDetail(db.Model):
    __tablename__ = 'nt_1_proformaservicebilldetail'
    Doc_No = Column(Integer, nullable=True)
    ...
    rbid = Column(Integer, ForeignKey('nt_1_proformaservicebillhead.rbid'), nullable=False)
    Group_Code = Column(Integer, nullable=True)
    gcid = Column(Integer, nullable=True)
