...
def get_indian_time():
    ...
    return datetime.now(IST).replace(microsecond=0).time()
...
class TenderDetails(db.Model):
    ...
    Sauda_Time = db.Column(db.Time, default=get_indian_time)
    gradeCode = db.Column(db.Integer, nullable=True)
    ...
    Purchase_Rate = db.Column(db.Numeric(18, 2), nullable=True)
    EbuySelectedParty =  db.Column(db.Integer)
    EbuySelectedAccoid =  db.Column(db.Integer)
    ebuyid =  db.Column(db.Integer)
    EbuySugarLiftingDate = db.Column(db.Date)
    Payment_To = db.Column(db.Integer)
    pt = db.Column(db.Integer)
...
class TenderGradeDetails(db.Model):
    __tablename__ = 'nt_1_tenderGradeDetails'

    id = db.Column(db.Integer, primary_key=True)
    gradeCode = db.Column(db.Integer, nullable=True)
    gradeid = db.Column(db.Integer, nullable=True)
    gradeRate = db.Column(db.Numeric(18, 2), nullable=True)
    tenderid = db.Column(db.Integer, db.ForeignKey('nt_1_tender.tenderid'), nullable=False)
    Purchase_Rate = db.Column(db.Numeric(18, 2), nullable=True)

    tender_info = relationship('TenderHead', backref='tendergradedetails', lazy=True)
