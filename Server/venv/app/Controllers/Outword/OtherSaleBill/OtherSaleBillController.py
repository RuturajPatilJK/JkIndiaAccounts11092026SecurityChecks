...
@app.route(API_URL + "/get-nextother-doc-no", methods=["GET"])
def get_nextother_doc_no():
    try:
...
@app.route(API_URL+"/getdata-otherSaleBill", methods=["GET"])
def getdata_otherSaleBill():
    try:
        company_code = request.args.get('Company_Code')
        year_code = request.args.get('Year_Code')
...
FROM     dbo.nt_1_accountmaster AS shipTo RIGHT OUTER JOIN
                 ...
                 where dbo.nt_1_sugarsale.Company_Code = :company_code and dbo.nt_1_sugarsale.Year_Code = :year_code and dbo.nt_1_sugarsale.Tran_Type ='SO' order by dbo.nt_1_sugarsale.doc_no desc
                                 '''
            )
...
@app.route(API_URL+"/otherSaleBillByid", methods=["GET"])
def getotherSaleBillByid():
    try:
...
#PR help
@app.route(API_URL+"/otherSaleBillByDocNo", methods=["GET"])
def otherSaleBillByDocNo():
    try:
...
@app.route(API_URL + "/insert-otherSaleBill", methods=["POST"])
def insert_otherSaleBill():
...
@app.route(API_URL + "/update-otherSaleBill", methods=["PUT"])
def update_otherSaleBill():     
    try:
        data = request.get_json()
        headData = data['headData']
        detailData = data['detailData']
...
@app.route(API_URL + "/delete_otherdata_by_saleid", methods=["DELETE"])
def delete_otherdata_by_saleid():
    try:
        saleid = request.args.get('saleid')
        Company_Code = request.args.get('Company_Code')
        doc_no = request.args.get('doc_no')
        ...
        if not all([Tran_Type,saleid, Company_Code, doc_no, Year_Code]):
            return jsonify({"error": "Missing required parameters"}), 400
        ...
        try:
            deleted_saleBillDetail_rows = SaleBillDetail.query.filter_by(saleid=saleid, Tran_Type=Tran_Type).delete()
            
            deleted_saleBillHead_rows = SaleBillHead.query.filter_by(saleid=saleid, Tran_Type=Tran_Type).delete()
...
@app.route(API_URL+"/get-firstotherSaleBill-navigation", methods=["GET"])
def get_firstotherSaleBill_navigation():
    try:

        Company_Code = request.args.get('Company_Code')
        Year_Code = request.args.get('Year_Code')
        Tran_Type = request.args.get('Tran_Type')
        if not all([Tran_Type,Company_Code, Year_Code]):
            return jsonify({"error": "Missing required parameters"}), 400
...
@app.route(API_URL+"/get-lastotherSaleBill-navigation", methods=["GET"])
def get_lastotherSaleBill_navigation():
    try:
...
@app.route(API_URL+"/get-previousotherSaleBill-navigation", methods=["GET"])
def get_previousotherSaleBill_navigation():
    try:
...
@app.route(API_URL+"/get-nextotherSaleBill-navigation", methods=["GET"])
def get_nextotherSaleBill_navigation():
    try:
        current_doc_no = request.args.get('currentDocNo')
        Company_Code = request.args.get('Company_Code')
        Year_Code = request.args.get('Year_Code')
...
@app.route(API_URL + "/Generate_otherSaleBill", methods=["PUT"])
def Generate_otherSaleBill():
    def get_max_doc_no(company_code,year_code):
...
@app.route(API_URL+"/generating_othersaleBill_report", methods=["GET"])
def generating_othersaleBill_report():
    try:
        company_code = request.args.get('Company_Code')
        year_code = request.args.get('Year_Code')
        doc_no = request.args.get('doc_no')
...
FROM            dbo.nt_1_companyparameters INNER JOIN
                 ...
                 where dbo.qrysalehead.Company_Code = :company_code and dbo.qrysalehead.Year_Code = :year_code and dbo.qrysalehead.doc_no = :doc_no and dbo.qrysalehead.Tran_Type = 'SO'
                                 '''
            )
...
@app.route(API_URL + "/DeleteotherTransactionSale", methods=["DELETE"])
def DeleteotherTransactionSale():
    try:
...
@app.route(API_URL+"/getotherEwayBillGeneratioData_SB", methods=["GET"])
def getotherEwayBillGeneratioData_SB():
    try:
        company_code = request.args.get('Company_Code')
        year_code = request.args.get('Year_Code')
        doc_no = request.args.get('doc_no')
...
WHERE        (dbo.NT_1_qryEwayBillCarporateSale.company_code = :company_code) AND (dbo.NT_1_qryEwayBillCarporateSale.DO_NO = :doc_no) AND (dbo.NT_1_qryEwayBillCarporateSale.Year_Code = :year_code)
            ...
            query = ('''SELECT        dbo.NT_1qryEwaybill.doc_no, CONVERT(varchar, dbo.NT_1qryEwaybill.doc_date, 103) AS doc_date, UPPER(dbo.NT_1qryEwaybill.BillToName) AS BillToName, UPPER(dbo.NT_1qryEwaybill.BillToGst) AS BillToGst, 
                            ...
                            dbo.nt_1_companyparameters.GSTStateCode AS fromStateCode,dbo.NT_1qryEwaybill.Pincode AS ShipToPinCode, dbo.company.PIN, dbo.NT_1qryEwaybill.billtopincode as BillToPinCode
...
@app.route(API_URL + "/get_eInvoice_othergenerationData_SB", methods=["GET"])
def get_eInvoice_othergenerationData_SB():
    try:
        doc_no = request.args.get('doc_no')
