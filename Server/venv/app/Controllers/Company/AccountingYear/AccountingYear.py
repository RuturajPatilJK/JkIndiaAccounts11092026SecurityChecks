...
def copy_instance(src, dest, exclude_keys=['id']):
    for key, attr in inspect(src).mapper.column_attrs.items():
...
def get_accounting_years():
    ...
    return jsonify([{
        ...
        'TCSApplicable': a.TCSApplicable,
        'newCompanyName': a.newCompanyName,
        'CNameUpdatedDate': a.CNameUpdatedDate,
        'oldFormerlyName': a.oldFormerlyName
...
def create_accounting_year():
    ...
    existing_parameters = db.session.query(CompanyParameters).filter_by(Company_Code=company_code, Year_Code=new_year_code).first()
    if not existing_parameters:
        most_recent_parameters = db.session.query(CompanyParameters).filter(CompanyParameters.Company_Code == company_code).order_by(CompanyParameters.Year_Code.desc()).first()
        if most_recent_parameters:
            new_parameters = CompanyParameters(Year_Code=new_year_code, Company_Code=company_code)
            copy_instance(most_recent_parameters, new_parameters, exclude_keys=['id', 'Year_Code'])
            db.session.add(new_parameters)

    existing_user = db.session.query(TblUser).filter_by(Company_Code=company_code).first()
    if not existing_user:
        users = db.session.query(TblUser).filter_by(Company_Code=company_code).all()
        for user in users:
            new_user = copy.deepcopy(user)
            new_user.Year_Code = new_year_code  
            db.session.add(new_user)

        # Copy user details
        user_details = db.session.query(TblUserDetail).filter_by(User_Id=user.User_Id, Company_Code=company_code).all()
        for detail in user_details:
            new_detail = copy.deepcopy(detail)
            new_detail.Year_Code = new_year_code
            new_detail.uid = new_user.uid 
            new_detail.User_Id = new_user.User_Id 
